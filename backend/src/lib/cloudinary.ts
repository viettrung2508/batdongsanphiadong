...
export function getCloudinaryPublicIdFromUrl(url: string | null | undefined) {
  if (!url) {
    return null;
  }
...
export function resolveCloudinaryPublicId(asset: AssetReference) {
  return asset.publicId ?? getCloudinaryPublicIdFromUrl(asset.url);
}

export function getRemovedAssetPublicIds(oldAssets: AssetReference[], nextUrls: string[]) {
