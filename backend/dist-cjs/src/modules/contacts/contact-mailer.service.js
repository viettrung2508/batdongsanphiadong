...
function createTransporter() {
    if (!env_js_1.env.SMTP_HOST || !env_js_1.env.SMTP_PORT || !env_js_1.env.SMTP_USER || !env_js_1.env.SMTP_PASS || !env_js_1.env.SMTP_FROM_EMAIL) {
        return null;
    }
    const smtpUser = env_js_1.env.SMTP_USER.trim();
    const smtpPass = env_js_1.env.SMTP_PASS.replace(/\s+/g, "");
    return nodemailer_1.default.createTransport({
        host: env_js_1.env.SMTP_HOST,
        port: env_js_1.env.SMTP_PORT,
        secure: env_js_1.env.SMTP_SECURE ?? env_js_1.env.SMTP_PORT === 465,
        auth: {
            user: smtpUser,
            pass: smtpPass
        }
...
async function sendContactNotificationEmail(notificationEmail, payload) {
    ...
    if (!transporter) {
        console.warn("[contact-email] skipped: SMTP is not configured", {
            contactId: payload.id,
            hasHost: Boolean(env_js_1.env.SMTP_HOST),
            hasPort: Boolean(env_js_1.env.SMTP_PORT),
            hasUser: Boolean(env_js_1.env.SMTP_USER),
            hasPass: Boolean(env_js_1.env.SMTP_PASS),
            hasFromEmail: Boolean(env_js_1.env.SMTP_FROM_EMAIL)
        });
        return;
    }
    console.info("[contact-email] sending", {
        ...
        from: env_js_1.env.SMTP_FROM_EMAIL,
        replyTo: payload.email ?? null
    });
    await transporter.sendMail({
        to: notificationEmail,
        from: env_js_1.env.SMTP_FROM_NAME
            ? `"${env_js_1.env.SMTP_FROM_NAME}" <${env_js_1.env.SMTP_FROM_EMAIL}>`
            : env_js_1.env.SMTP_FROM_EMAIL,
